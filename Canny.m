...
clc;
%Input image
srcFile=dir('C:\Users\sanchit\Desktop\Fingerprint_Dataset\save\Kaushik\Skin\*.jpg');
for r=1:length(srcFile)
path = strcat('C:\Users\sanchit\Desktop\Fingerprint_Dataset\save\Kaushik\Skin\',srcFile(r).name);
img = imread(path);
%img = imread ('F:\CLG WORK\T.E. ETRX\ARP\Contactless_Fingerprint\Skin_detected.jpg');
%Show input image
%figure, imshow(img);
img = rgb2gray(img);
img = double (img);
...
database = strcat('C:\Users\sanchit\Desktop\Fingerprint_Dataset\save\Kaushik\Edge\',srcFile(r).name);
imwrite(BW,database);
end